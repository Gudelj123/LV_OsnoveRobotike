...
def task5(map_file_name, start, goal, map_resolution, robot_radius):
    ...
    nnf = -np.ones(map.shape).astype('int32')
    max_dist = nnf.shape[0] * nnf.shape[1]
    nnf[1:-1,1:-1] = max_dist
    nnf[np.invert(free_space)] = -1
    start = np.round(start / map_resolution).astype('int32')
    goal = np.round(goal / map_resolution).astype('int32')
    nnf[goal[1], goal[0]] = 0
    search_completed = False
    dist = 0
    while not search_completed:
        y, x = np.nonzero(nnf == dist)
        if x.shape[0] > 0:
            if np.count_nonzero(np.logical_and(x == start[0], y == start[1])) > 0:
                search_completed = True
                path_found = True
            else:
                new_dist = dist + 1
                nnf[y,x+1] = np.minimum(nnf[y,x+1], new_dist)
                nnf[y+1,x] = np.minimum(nnf[y+1,x], new_dist)
                nnf[y,x-1] = np.minimum(nnf[y,x-1], new_dist)
                nnf[y-1,x] = np.minimum(nnf[y-1,x], new_dist)
                dist = new_dist
        else:
    ...
    if path_found:
        ...
        path = np.array(path)
        map_with_path[path[:,1], path[:,0]] = 0.5
...
def main():
    ...
    #task3()
    #task4()
    path = task5('/media/petra/My Passport/Nastava/OR/Python/map.png', np.array([28,16]), np.array([22, 16]), 0.04, 0.2) #*0.04, 0.04, 0.2
    task6('/media/petra/My Passport/Nastava/OR/Python/map.png', path, 150)
